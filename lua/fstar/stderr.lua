...
local function open_window(stderr_bufnr)
  ...
  vim.api.nvim_buf_set_option(stderr_bufnr, 'filetype', 'fstarstderr')
  vim.api.nvim_set_current_win(old_win)
  return stderr_winnr
end

local function create_buf(params)
...
function stderr.show(message)
  vim.schedule(function()
    if not current.bufnr or not vim.api.nvim_buf_is_valid(current.bufnr) then
      current.bufnr = create_buf { name = 'fstar://stderr', listed = false, scratch = true }
      current.winnr = nil
    end
...
function stderr.enable(config)
  ...
  log.error = function(...)
    ...
    if
      ...
      and select(1, ...) == 'rpc'
      and select(3, ...) == 'stderr'
      and string.match(select(2, ...), 'fstar')
    then
